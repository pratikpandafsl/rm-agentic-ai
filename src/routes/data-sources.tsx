...
export const Route = createFileRoute("/data-sources")({
  head: () => ({
    meta: [
      { title: "Data Sources · RM Agentic AI — Mayo Phase 1" },
      {
        name: "description",
        content:
          "Register the four approved Mayo extracts with period, checksum, classification and lineage.",
      },
      { property: "og:title", content: "Data Sources · RM Agentic AI — Mayo Phase 1" },
      {
...
type Source = {
  name: string;
  system: string;
  status: "Ready" | "Access check" | "Pending";
  rows?: string;
};
  ...
  return (
    <AppLayout active="Data Sources">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Register the four approved Mayo extracts with period, checksum,
        ...
        {SOURCES.map((source) => (
          ...
          >
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              {source.rows && (
                <span className="text-xs text-muted-foreground">{source.rows}</span>
              )}
            </div>
          </div>
        ))}
        ...
        <div className="p-5">
          ...
          >
            <span className="text-sm font-semibold text-foreground">
              Drop or browse approved masked files
            </span>
            <span className="mt-1.5 text-xs text-muted-foreground">
              CSV, XLSX or JSON. Direct identifiers must not enter model requests.
            </span>
            {notice && (
              <span className="mt-3 text-xs font-medium text-green-700">
                {notice}
              </span>
            )}
          </button>
