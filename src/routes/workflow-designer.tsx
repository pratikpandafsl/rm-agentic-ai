...
export const Route = createFileRoute("/workflow-designer")({
  head: () => ({
    meta: [
      { title: "Workflow Designer · RM Agentic AI" },
      { name: "description", content: "Configure the ordered evidence workflow, retries, failure route and human review gate." },
      { property: "og:title", content: "Workflow Designer · RM Agentic AI" },
      { property: "og:description", content: "Configure the ordered evidence workflow, retries, failure route and human review gate." },
...
function WorkflowDesignerRoute() {
  const ready = useProtectedPage("Workflow Designer");
...
function WorkflowDesignerPage() {
  const { tenant } = useApp();
  const [agents, setAgents] = useState(tenant.workflow.agents);
  const [notice, setNotice] = useState<string | null>(null);

  useEffect(() => {
    setAgents(tenant.workflow.agents);
    setNotice(null);
  }, [tenant]);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Configure the ordered {tenant.shortName} sequence, retries, failure route and HITL gate.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => flash(`${tenant.workflow.name} published.`)} className="rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Publish</button>
        </div>
      </div>

      <section className="mt-4 overflow-hidden rounded-lg border border-border bg-card shadow-sm">
        <h2 className="border-b border-border px-4 py-3 text-base font-semibold text-foreground">{tenant.workflow.name}</h2>
